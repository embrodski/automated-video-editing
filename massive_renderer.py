...
By default, interview DSLs render Ben / Guest / Wide and reading DSLs render
Front / Side. Derived encodes run **sequentially** to keep CPU/GPU and disk load
predictable.
"""
...
def is_reading_dsl(dsl_text: str) -> bool:
    """Return True for DSLs generated by generate_reading_dsl.py."""
    return is_reading_dsl_text(dsl_text)


def variant_specs_for_dsl(
    dsl_text: str,
    *,
    ben_name: str = "Ben Render",
    guest_name: str = "Guest Render",
    wide_name: str = "Wide Render",
    front_name: str = "Front Render",
    side_name: str = "Side Render",
) -> List[VariantSpec]:
    if is_reading_dsl(dsl_text):
        return [
            VariantSpec(front_name, "speaker_0"),
            VariantSpec(side_name, "speaker_1"),
        ]

    return [
        VariantSpec(ben_name, "speaker_0"),
        VariantSpec(guest_name, "speaker_1"),
        VariantSpec(wide_name, "wide"),
    ]
...
def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Create single-camera DSL variants from a base DSL and render them sequentially. "
            "Interview DSLs render Ben/Guest/Wide; reading DSLs render Front/Side."
        )
    )
    ...
    p.add_argument(
        ...
        default=None,
        help="Optional encoder override (defaults to podcast_dsl default: libx264).",
    )
    p.add_argument(
        "--hardware-encode",
        action="store_true",
        help="Forward --hardware-encode to podcast_dsl (try hardware encoders with libx264 fallback).",
    )
    ...
    p.add_argument(
        "--front-name",
        default="Front Render",
        help='Output base name for the reading front-camera render (default: "Front Render").',
    )
    p.add_argument(
        "--side-name",
        default="Side Render",
        help='Output base name for the reading side-camera render (default: "Side Render").',
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    specs = variant_specs_for_dsl(
        base_text,
        ben_name=str(args.ben_name),
        guest_name=str(args.guest_name),
        wide_name=str(args.wide_name),
        front_name=str(args.front_name),
        side_name=str(args.side_name),
    )
    ...
    for spec in specs:
        ...
        jobs.append(RenderJob(name=spec.name, camera=spec.camera, dsl_path=dsl_out, out_path=mp4_out))

    # Sequential encodes: multiple full-timeline renders at once overload many machines.
    failures: List[Tuple[RenderJob, int]] = []
    sequence = ", then ".join(job.name for job in jobs)
    print(
        f"Massive: rendering {sequence} (one encode at a time)...",
        flush=True,
    )
    for job in jobs:
        ...
        _, code = _run_render_job(
            ...
            hardware_encode=bool(args.hardware_encode),
