...
class FixAudioSpeakerSwapTests(unittest.TestCase):
    def _write_state(self, root: Path, *, swap_speaker_ids: bool = False) -> None:
        ...
        state = {
            ...
            "paths": {
                ...
                "temp": str(root / "Temp"),
                "state": str(root / PIAB_STATE_FILENAME),
            },
            "main_prepped": {
                "prepped_videos": [
    ...
    def test_toggles_swap_speaker_ids_and_rerenders(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            state = json.loads((root / PIAB_STATE_FILENAME).read_text(encoding="utf-8"))
    ...
    def test_requires_prep_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / PIAB_STATE_FILENAME).write_text(
                json.dumps(
                    {
                        "kind": "podcast_in_a_box",
                        "paths": {"state": str(root / PIAB_STATE_FILENAME)},
                        "steps": {},
                    }
                ),
                encoding="utf-8",
